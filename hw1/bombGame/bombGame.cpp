...
long long int fastCombine(vector<EVENT> &eventSet, long long int &front_idx, long long int &mid_idx, long long int &end_idx, vector<long long int>  &damage){
    ...
    //get player and sort
    for (long long int j=front_idx; j<mid_idx+1; j++){
        if (eventSet[j].eventType==0){
            tempP.push_back(eventSet[j]);
        } 
    }
    ...
    //get attack and sort
    for (long long int i=mid_idx+1; i<end_idx+1; i++){
        if (eventSet[i].eventType==1){
            tempA.push_back(eventSet[i]);
        } 
    }
