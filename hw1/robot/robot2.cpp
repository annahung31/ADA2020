...
long long int dp[12][505][505];
long long int farm[505][505];
...
void updateMAX(vector< MAXSWEET>& maxSweet, long long d, int i, int j){
...
RESULT getSweet(int n, int m, long long int k, long long int c){
    STEP step;
    ...
    while (i >= 1 && j >= 1){
        ...
        if (currMax == maxSweet[d-1].sweetness){
            output.moves.push_back(recordMove("Jump", i, j));
            d--;
            i = maxSweet[d].x;
            j = maxSweet[d].y;
            // cout << "(" << i << "," << j << ")" << endl;
...
int main(){ 
    ...
    output = getSweet(n ,m, k, c);
    cout << output.sweetness << endl;
    cout << output.nMove << endl;
    for (i=output.moves.size()-1; i>=0; i--){
        cout << output.moves[i].moveType << " " << output.moves[i].moveI-1 << " " << output.moves[i].moveJ-1 << '\n';
    }
